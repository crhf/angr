...
def run_keystone(arch):
    ...
    unittest.TestCase().assertEqual(block.instructions, 1)

    sm.step(force_addr=addr, insn_text=insn_texts[arch], thumb=is_thumb)

    if arch in ['i386', 'x86_64']:
        unittest.TestCase().assertEqual(sm.one_active.solver.eval(sm.one_active.regs.eax), 0x12)
    else:
        unittest.TestCase().assertEqual(sm.one_active.solver.eval(sm.one_active.regs.r1), 0x12)
