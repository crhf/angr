...
def test_alignment():
    for arch in all_arches:
        if arch.name in DEFAULT_CC and DEFAULT_CC[arch.name] is not SimCCUnknown:
            ...
            unittest.TestCase().assertTrue(st.solver.is_true(((st.regs.sp + cc.STACKARG_SP_DIFF) % cc.STACK_ALIGNMENT == 0)),
...
def test_sys_v_abi_compliance():
    ...
    unittest.TestCase().assertTrue(st.solver.is_true(((st.regs.rsp + 8) % 16 == 0)),
