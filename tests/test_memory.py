...
def test_copy():
    ...
    unittest.TestCase().assertEqual(sorted(s.solver.eval_upto(x, 100)), list(range(10)))
    result = s.memory.load(0x200, 5)
    unittest.TestCase().assertEqual(sorted(s.solver.eval_upto(result, 100, cast_to=bytes)), [ b"ABCDE", b"ABCDX", b"ABCXX", b"ABXXX", b"AXXXX", b"XXXXX" ])
    unittest.TestCase().assertEqual(sorted(s.solver.eval_upto(result, 100, cast_to=bytes, extra_constraints=[x==3])), [ b"ABCXX" ])
    ...
    unittest.TestCase().assertEqual(sorted(s.solver.eval_upto(ret_x, 100)), list(range(10)))
    unittest.TestCase().assertEqual(sorted(s.solver.eval_upto(result, 100, cast_to=bytes, extra_constraints=[ret_x==3])), [ b"ABCXX" ])
...
def _concrete_memory_tests(s):
    ...
    unittest.TestCase().assertIs(expr, s.solver.BVV(0x1337, 32))
    expr = s.memory.load(100, 2)
    unittest.TestCase().assertIs(expr, s.solver.BVV(0, 16))
    expr = s.memory.load(102, 2)
    unittest.TestCase().assertIs(expr, s.solver.BVV(0x1337, 16))
    ...
    unittest.TestCase().assertIs(expr, s.solver.BVV(0x141537, 24))
    expr = s.memory.load(100, 2)
    assert s.solver.min(expr) == 0x14
    expr = s.memory.load(102, 2)
    unittest.TestCase().assertIs(expr, s.solver.BVV(0x1537, 16))
    expr = s.memory.load(102, 2, endness="Iend_LE")
    unittest.TestCase().assertIs(expr, s.solver.BVV(0x3715, 16))
...
def test_memory():
    ...
    expr = s.memory.load(2, 4)
    expr = s.memory.load(2, 4)
    unittest.TestCase().assertTrue(s.solver.symbolic(expr))
    unittest.TestCase().assertFalse(s.solver.unique(expr))
    unittest.TestCase().assertGreaterEqual(s.solver.eval(expr), 0x41410000)
    unittest.TestCase().assertLessEqual(s.solver.eval(expr), 0x41420000)
    unittest.TestCase().assertEqual(s.solver.min_int(expr), 0x41410000)
    unittest.TestCase().assertEqual(s.solver.max_int(expr), 0x4141ffff)

    # concrete address and concrete result
    expr = s.memory.load(0, 4) # Returns: a z3 BVS representing 0x41414141
    unittest.TestCase().assertFalse(s.solver.symbolic(expr))
    unittest.TestCase().assertEqual(s.solver.eval(expr), 0x41414141)

    c = s.solver.BVS('condition', 8)
    expr = s.memory.load(10, 1, condition=c==1, fallback=s.solver.BVV(b'X'))
    unittest.TestCase().assertEqual(s.solver.eval_upto(expr, 10, cast_to=bytes, extra_constraints=[c==1]), [ b'B' ])
    unittest.TestCase().assertEqual(s.solver.eval_upto(expr, 10, cast_to=bytes, extra_constraints=[c!=1]), [ b'X' ])

    x = s.solver.BVS('ref_test', 16, explicit_name=True)
    s.memory.store(0x1000, x)
    s.memory.store(0x2000, x)
    unittest.TestCase().assertEqual(set(s.memory.addrs_for_name('ref_test')), {0x1000,0x1001,0x2000,0x2001})
    unittest.TestCase().assertEqual(set(s.memory.addrs_for_hash(x.cache_key)), {0x1000, 0x1001, 0x2000, 0x2001})
    ...
    unittest.TestCase().assertEqual(set(s.memory.addrs_for_name('ref_test')), {0x1000,0x1001,0x2000,0x2001})
    unittest.TestCase().assertEqual(set(s.memory.addrs_for_hash(x.cache_key)), {0x1000,0x1001,0x2000,0x2001})
    unittest.TestCase().assertEqual(set(s2.memory.addrs_for_name('ref_test')), {0x1000, 0x1001})
    unittest.TestCase().assertEqual(set(s2.memory.addrs_for_hash(x.cache_key)), {0x1000, 0x1001})
    unittest.TestCase().assertEqual(set(s2.memory.addrs_for_name('ref_test2')), {0x2000, 0x2001})
    unittest.TestCase().assertEqual(set(s2.memory.addrs_for_hash(y.cache_key)), {0x2000, 0x2001})

    s.memory.store(0x3000, s.solver.BVS('replace_old', 32, explicit_name=True))
    s.memory.store(0x3001, s.solver.BVV(b'AB'))
    unittest.TestCase().assertEqual(set(s.memory.addrs_for_name('replace_old')), {0x3000, 0x3003})
    unittest.TestCase().assertEqual(s.solver.eval_upto(s.memory.load(0x3001, 2), 10, cast_to=bytes), [b"AB"])
    ...
    #unittest.TestCase().assertEqual(len(mo), 1)
    #s.memory.replace_memory_object(next(iter(mo)), n)
    #unittest.TestCase().assertEqual(set(s.memory.addrs_for_name('replace_old')), set())
    #unittest.TestCase().assertEqual(set(s.memory.addrs_for_name('replace_new')), {0x3000, 0x3003})
    #unittest.TestCase().assertEqual(s.solver.eval_upto(s.memory.load(0x3001, 2), 10, cast_to=bytes), [b"AB"])

    #s.memory.store(0x4000, s.solver.If(n == 0, n+10, n+20))

    #unittest.TestCase().assertEqual(set(s.memory.addrs_for_name('replace_new')), {0x3000, 0x3003, 0x4000, 0x4001, 0x4002, 0x4003})
    #s.memory.replace_all(n, c)
    #unittest.TestCase().assertEqual(set(s.memory.addrs_for_name('replace_old')), set())
    #unittest.TestCase().assertEqual(set(s.memory.addrs_for_name('replace_new')), set())
    #unittest.TestCase().assertEqual(set(s.memory.addrs_for_name('replace_cool')), {0x3000, 0x3003, 0x4000, 0x4001, 0x4002, 0x4003})
    #unittest.TestCase().assertEqual(s.solver.eval_upto(s.memory.load(0x3001, 2), 10, cast_to=bytes), [b"AB"])
    ...
    #unittest.TestCase().assertEqual(set(s.memory.addrs_for_name('replace_old')), set())
    #unittest.TestCase().assertEqual(set(s.memory.addrs_for_name('replace_new')), set())
    #unittest.TestCase().assertEqual(set(s.memory.addrs_for_name('replace_cool')), set())
    #unittest.TestCase().assertEqual(s.solver.eval_upto(s.memory.load(0x3001, 2), 10, cast_to=bytes), [b"AB"])
    #unittest.TestCase().assertEqual(s.solver.eval_upto(s.memory.load(0x3000, 4), 10), [0x00414200])
    #unittest.TestCase().assertEqual(s.solver.eval_upto(s.memory.load(0x4000, 4), 10), [0x0000000a])
    ...
    unittest.TestCase().assertEqual(set(s.solver.eval_upto(s.memory.load(0x5000, 4), 10)), { 0x11223344, 0xAA223344, 0xAABB3344, 0xAABBCC44, 0xAABBCCDD })

    s1 = s.copy()
    s1.add_constraints(n == 1)
    unittest.TestCase().assertEqual(set(s1.solver.eval_upto(s1.memory.load(0x5000, 4), 10)), { 0xAA223344 })

    s4 = s.copy()
    s4.add_constraints(n == 4)
    unittest.TestCase().assertEqual(set(s4.solver.eval_upto(s4.memory.load(0x5000, 4), 10)), { 0xAABBCCDD })
    ...
    unittest.TestCase().assertEqual(set(s.solver.eval_upto(s.memory.load(0x6000, 4), 10)), { 0x11223344, 0xAABBCCDD })

    s0 = s.copy()
    s0.add_constraints(c == 0)
    unittest.TestCase().assertEqual(set(s0.solver.eval_upto(s0.memory.load(0x6000, 4), 10)), { 0x11223344 })

    s1 = s.copy()
    s1.add_constraints(c == 1)
    unittest.TestCase().assertEqual(set(s1.solver.eval_upto(s1.memory.load(0x6000, 4), 10)), { 0xAABBCCDD })
    ...
    s0 = s.copy()
    s0.add_constraints(c == 0)
    unittest.TestCase().assertEqual(set(s0.solver.eval_upto(s0.memory.load(0x8000, 4), 10)), { 0x11223344 })

    s1 = s.copy()
    s1.add_constraints(c == 1)
    unittest.TestCase().assertEqual(set(s1.solver.eval_upto(s1.memory.load(0x8000, 4), 10)), { 0x11223344, 0xAA223344, 0xAABB3344, 0xAABBCC44, 0xAABBCCDD })
...
def test_abstract_memory():
    ...
    unittest.TestCase().assertEqual(s.solver.eval(expr), 0x43)
    unittest.TestCase().assertEqual(s.solver.max_int(expr), 0x43)
    unittest.TestCase().assertEqual(s.solver.min_int(expr), 0x43)
    ...
    unittest.TestCase().assertEqual(s.solver.eval(expr), 0x44)
    ...
    unittest.TestCase().assertEqual(s.solver.min_int(expr), 10)
    unittest.TestCase().assertEqual(s.solver.max_int(expr), 20)
    unittest.TestCase().assertEqual(s.solver.eval_upto(expr, 100), [10, 12, 14, 16, 18, 20])
    ...
    unittest.TestCase().assertTrue(claripy.backends.vsa.identical(expr, si_1))
    ...
    unittest.TestCase().assertTrue(claripy.backends.vsa.identical(expr, s.solver.BVS('unnamed', 32, 8000, 9000, 2)))

    # Test default values
    s.options.remove(o.SYMBOLIC_INITIAL_VALUES)
    expr = s.memory.load(to_vs('global', 100), 4)
    unittest.TestCase().assertTrue(claripy.backends.vsa.identical(expr, s.solver.BVS('unnamed', 32, 0, 0, 0)))

    # Test default values (symbolic)
    s.options.add(o.SYMBOLIC_INITIAL_VALUES)
    expr = s.memory.load(to_vs('global', 104), 4)
    unittest.TestCase().assertTrue(claripy.backends.vsa.identical(expr, s.solver.BVS('unnamed', 32, 0, 0xffffffff, 1)))
    unittest.TestCase().assertTrue(claripy.backends.vsa.identical(expr, s.solver.BVS('unnamed', 32, -0x80000000, 0x7fffffff, 1)))
    ...
    unittest.TestCase().assertTrue(claripy.backends.vsa.identical(expr, s.solver.BVS('unnamed', 8, 0x10, 0x20, 0x10)))
    ...
    unittest.TestCase().assertTrue(claripy.backends.vsa.identical(expr, se.SI(bits=32, stride=1, lower_bound=0x100000, upper_bound=0x100001)))
    ...
    unittest.TestCase().assertEqual(len(object_set), 1)
    ...
    c = a.merge(b)[0]
    expr = c.memory.load(to_vs('function_merge', 0x20), 4)
    unittest.TestCase().assertTrue(claripy.backends.vsa.identical(expr, se.SI(bits=32, stride=0x100000, lower_bound=0x100000, upper_bound=0x300000)))
    object_set = {c_page._get_object(0x20, 0),
                  c_page._get_object(0x21, 0),
                  c_page._get_object(0x22, 0),
                  c_page._get_object(0x23, 0),
...
def test_abstract_memory_find():
    ...
    unittest.TestCase().assertTrue(isinstance(r_model, claripy.vsa.ValueSet))
    unittest.TestCase().assertEqual(list(r_model.regions.keys()), [ 'global' ])
    unittest.TestCase().assertTrue(claripy.backends.vsa.identical(r_model.regions['global'], s_expected))
...
def test_registers():
    ...
    expr = s.registers.load('rax')
    unittest.TestCase().assertFalse(s.solver.symbolic(expr))
    unittest.TestCase().assertEqual(s.solver.eval(expr), 0x00000031)
...
def test_concrete_memset():
    def _individual_test(state, base, val, size):
        ...
        unittest.TestCase().assertLessEqual(elapsed, 5)
        # the result should be good
        byt_0 = memset.state.memory.load(base, 1)
        unittest.TestCase().assertEqual(s.solver.eval_upto(byt_0, 10), [val])
        byt_1 = memset.state.memory.load(base+1, 1)
        unittest.TestCase().assertEqual(s.solver.eval_upto(byt_1, 10), [val])
        byt_2 = memset.state.memory.load(base+size-1, 1)
        unittest.TestCase().assertEqual(s.solver.eval_upto(byt_2, 10), [val])
