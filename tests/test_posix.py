...
def test_file_create():
    ...
    unittest.TestCase().assertEqual(fd, 3)
...
def test_file_read():
    ...
    unittest.TestCase().assertIs(data, content)
...
def test_file_seek():
    ...
    simfd = state.posix.get_fd(fd)
    simfd.seek(0, 'start')
    unittest.TestCase().assertTrue(state.solver.is_true(simfd.tell() == 0))
    ...
    unittest.TestCase().assertTrue(state.solver.is_false(state.posix.get_fd(0).seek(0)))
    unittest.TestCase().assertTrue(state.solver.is_false(state.posix.get_fd(1).seek(0)))
    unittest.TestCase().assertTrue(state.solver.is_false(state.posix.get_fd(2).seek(0)))
    ...
    simfd = state.posix.get_fd(fd)
    simfd.seek(0, 'end')
    unittest.TestCase().assertTrue(state.solver.is_true(simfd.tell() == 20))
    state.posix.close(fd)
    ...
    simfd.seek(0, 'end')
    unittest.TestCase().assertIs(real_end, simfd.tell())
    state.posix.close(fd)
