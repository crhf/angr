...
def test_multi_open_file():
    ...
    for p in pg.deadended:
        unittest.TestCase().assertTrue(p.posix.dumps(2) == b"")

        # Check that the temp file was deleted
        unittest.TestCase().assertEqual(p.fs._files, {})
        ...
        for event in p.history.events:
            if event.type == 'fs_unlink':
                ...
                unittest.TestCase().assertEqual(simfile.concretize(), b'foobar and baz')
                break
        else:
            assert False
