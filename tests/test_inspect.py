...
#       counts.constraints += 1
    ...
    unittest.TestCase().assertEqual(counts.mem_write, 0)
    s.memory.store(100, s.solver.BVV(10, 32))
    unittest.TestCase().assertEqual(counts.mem_write, 1)
    ...
    unittest.TestCase().assertEqual(counts.mem_read, 0)
    s.memory.load(123, 4)
    s.memory.load(223, 3)
    unittest.TestCase().assertEqual(counts.mem_read, 4)

    s.inspect.b('reg_read', when=BP_AFTER, action=act_reg_read)
    unittest.TestCase().assertEqual(counts.reg_read, 0)
    s.registers.load(16, size=8)
    unittest.TestCase().assertEqual(counts.reg_read, 1)

    s.inspect.b('reg_write', when=BP_AFTER, action=act_reg_write)
    unittest.TestCase().assertEqual(counts.reg_write, 0)
    s.registers.store(16, s.solver.BVV(10, 32))
    unittest.TestCase().assertEqual(counts.reg_write, 1)
    unittest.TestCase().assertEqual(counts.mem_write, 1)
    unittest.TestCase().assertEqual(counts.mem_read, 4)
    unittest.TestCase().assertEqual(counts.reg_read, 1)
    ...
    unittest.TestCase().assertEqual(counts.reg_write, 7)
    unittest.TestCase().assertEqual(counts.reg_read, 2)
    unittest.TestCase().assertEqual(counts.tmp_write, 1)
    unittest.TestCase().assertEqual(counts.tmp_read, 1)
    unittest.TestCase().assertEqual(counts.expr, 3) # one for the Put, one for the WrTmp, and one to get the next address to jump to
    unittest.TestCase().assertEqual(counts.statement, 11)
    unittest.TestCase().assertEqual(counts.instruction, 2)
    unittest.TestCase().assertEqual(counts.constraints, 0)
    unittest.TestCase().assertEqual(counts.mem_write, 1)
    unittest.TestCase().assertEqual(counts.mem_read, 4)
    ...
    s.memory.load(0, 10)
    unittest.TestCase().assertEqual(counts.variables, 1)
...
def test_inspect_exit():
    ...
    def handle_exit_before(state):
        ...
        unittest.TestCase().assertEqual(state.solver.eval(exit_target), 0x3f8)
        # change exit target
        state.inspect.exit_target = 0x41414141
        unittest.TestCase().assertEqual(state.inspect.exit_jumpkind, "Ijk_Boring")
        unittest.TestCase().assertTrue(state.inspect.exit_guard.is_true())
    ...
    # check
    unittest.TestCase().assertEqual( succ[0].solver.eval(succ[0].ip), 0x41414141)
    unittest.TestCase().assertEqual(counts.exit_before, 1)
    unittest.TestCase().assertEqual(counts.exit_after, 1)
...
def test_inspect_syscall():
    ...
    def handle_syscall_before(state):
        ...
        unittest.TestCase().assertEqual(syscall_name, "close")
...
def test_inspect_engine_process():
    ...
    def check_first_symbolic_fork(state):
        ...
        unittest.TestCase().assertEqual(len(succ_addr), 2)
        unittest.TestCase().assertIn('0x400692L', succ_addr)
        unittest.TestCase().assertIn('0x400699L', succ_addr)
    ...
    def check_second_symbolic_fork(state):
        ...
        succ_addr = [hex(s.addr) for s in succs]
        unittest.TestCase().assertEqual(len(succ_addr), 2)
        unittest.TestCase().assertIn('0x4006dfL', succ_addr)
        unittest.TestCase().assertIn('0x4006e6L', succ_addr)
        print('Fork after:', hex(state.addr))
        print('Successors:', succ_addr)
    ...
    def check_state(state):
        unittest.TestCase().assertIn(hex(state.inspect.sim_successors.addr), ('0x40068eL', '0x4006dbL'))
