...
def run_fauxware(arch, threads):
    ...
    unittest.TestCase().assertEqual(len(pg.active), 1)
    unittest.TestCase().assertEqual(pg.active[0].history.depth, 0)

    # step until the backdoor split occurs
    pg2 = pg.step(until=lambda lpg: len(lpg.active) > 1, step_func=lambda lpg: lpg.prune())
    unittest.TestCase().assertEqual(len(pg2.active), 2)
    unittest.TestCase().assertTrue(any(b"SOSNEAKY" in s for s in pg2.mp_active.posix.dumps(0).mp_items))
    unittest.TestCase().assertFalse(all(b"SOSNEAKY" in s for s in pg2.mp_active.posix.dumps(0).mp_items))
    ...
    unittest.TestCase().assertEqual(len(pg3.active), 0)
    unittest.TestCase().assertEqual(len(pg3.backdoor), 1)
    unittest.TestCase().assertEqual(len(pg3.auth), 1)
    ...
    unittest.TestCase().assertEqual(len(pg4.active), 0)
    unittest.TestCase().assertEqual(len(pg4.backdoor), 1)
    unittest.TestCase().assertEqual(len(pg4.auth), 1)
    ...
    unittest.TestCase().assertEqual(len(pg5.active), 0)
    unittest.TestCase().assertEqual(len(pg5.backdoor), 1)
    unittest.TestCase().assertEqual(len(pg5.auth), 2)

    # now unstash everything
    pg6 = pg5.unstash(from_stash='backdoor').unstash(from_stash='auth')
    unittest.TestCase().assertEqual(len(pg6.active), 3)
    unittest.TestCase().assertEqual(len(pg6.backdoor), 0)
    unittest.TestCase().assertEqual(len(pg6.auth), 0)

    unittest.TestCase().assertEqual(len(set(pg6.mp_active.addr.mp_items)), 1)

    # now merge them!
    pg7 = pg6.merge()
    unittest.TestCase().assertEqual(len(pg7.active), 2)
    unittest.TestCase().assertEqual(len(pg7.backdoor), 0)
    unittest.TestCase().assertEqual(len(pg7.auth), 0)
    ...
    unittest.TestCase().assertIs(st2, pg8.active[1])
    unittest.TestCase().assertIs_not(st1, pg8.active[0])

    total_active = len(pg8.active)

    # test special stashes
    unittest.TestCase().assertEqual(len(pg8.stashes['stashed']), 0)
    pg8.stash(filter_func=lambda p: p is pg8.active[1], to_stash='asdf')
    unittest.TestCase().assertEqual(len(pg8.stashes['stashed']), 0)
    unittest.TestCase().assertEqual(len(pg8.asdf), 1)
    unittest.TestCase().assertEqual(len(pg8.active), total_active-1)
    pg8.stash(from_stash=pg8.ALL, to_stash='fdsa')
    unittest.TestCase().assertEqual(len(pg8.asdf), 0)
    unittest.TestCase().assertEqual(len(pg8.active), 0)
    unittest.TestCase().assertEqual(len(pg8.fdsa), total_active)
    pg8.stash(from_stash=pg8.ALL, to_stash=pg8.DROP)
    unittest.TestCase().assertTrue(all(len(s) == 0 for s in pg8.stashes.values()))
...
def test_find_to_middle():
    ...
    unittest.TestCase().assertEqual(len(pg.found), 1)
    unittest.TestCase().assertTrue(pg.found[0].addr == 0x4006ee)
...
def test_explore_with_cfg():
    ...
    unittest.TestCase().assertEqual(len(pg.avoid), 1)
    unittest.TestCase().assertEqual(len(pg.found), 2)
    unittest.TestCase().assertEqual(pg.found[0].addr, 0x4006ED)
    unittest.TestCase().assertEqual(pg.found[1].addr, 0x4006ED)
    unittest.TestCase().assertEqual(pg.avoid[0].addr, 0x4007C9)
