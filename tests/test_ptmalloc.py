...
def run_malloc_maximizes_sym_arg(arch):
    ...
    unittest.TestCase().assertTrue(same_heap_states(s, sc))
...
def run_malloc_no_space_returns_null(arch):
    ...
    unittest.TestCase().assertEqual(p1, 0)
    unittest.TestCase().assertTrue(same_heap_states(s, sc))
...
def run_realloc_no_space_returns_null(arch):
    ...
    unittest.TestCase().assertEqual(p2, 0)
    unittest.TestCase().assertTrue(same_heap_states(s, sc))
...
def run_first_fit_and_free_malloced_makes_available(arch):
    ...
    s.heap.free(p1)
    p2 = s.heap.malloc(30)
    unittest.TestCase().assertEqual(p1, p2)
...
def run_realloc_moves_and_frees(arch):
    ...
    p3 = s.heap.malloc(30)
    unittest.TestCase().assertEqual(p1, p3)
    unittest.TestCase().assertLess(p1, p2)
...
def run_unusable_amount_returns_null(arch):
    ...
    sc = s.copy()
    p = s.heap.malloc(1)
    unittest.TestCase().assertEqual(p, 0)
    unittest.TestCase().assertTrue(same_heap_states(s, sc))
...
def run_free_null_preserves_state(arch):
    ...
    s2 = s.copy()
    s2.heap.free(0)
    unittest.TestCase().assertTrue(same_heap_states(s, s2))
...
def run_skips_chunks_too_small(arch):
    ...
    unittest.TestCase().assertLess(p, p2)
...
def run_calloc_clears(arch):
    ...
    unittest.TestCase().assertTrue(s.solver.is_true(v1 == 0))
    unittest.TestCase().assertTrue(sc.solver.is_true(v2 == -1))
