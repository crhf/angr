...
def test_lwip_udpecho_bm():
    ...
    unittest.TestCase().assertEqual(len(timenow_cp_xrefs), 1)
    unittest.TestCase().assertEqual(next(iter(timenow_cp_xrefs)),
                            XRef(ins_addr=0x23c9, dst=0x23d4, xref_type=XRefType.Read)
                            )

    unittest.TestCase().assertEqual(len(timenow_xrefs), 3)
    unittest.TestCase().assertEqual([x for x in timenow_xrefs if x.type == XRefType.Offset][0],
                            XRef(ins_addr=0x23c9, dst=0x1fff36f4, xref_type=XRefType.Offset)
                            )
    unittest.TestCase().assertEqual([x for x in timenow_xrefs if x.type == XRefType.Read][0],
                            XRef(ins_addr=0x23cb, dst=0x1fff36f4, xref_type=XRefType.Read)
                            )
    unittest.TestCase().assertEqual([x for x in timenow_xrefs if x.type == XRefType.Write][0],
...
def test_lwip_udpecho_bm_the_better_way():
    ...
    unittest.TestCase().assertEqual(len(timenow_xrefs), 5)
...
def test_p2im_drone_with_inits():
    ...
    unittest.TestCase().assertEqual(len(h12c1_inst_xrefs), 5)
