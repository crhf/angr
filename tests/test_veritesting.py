...
def run_veritesting_a(arch):
    ...
    unittest.TestCase().assertNotEqual(len(ex.found), 0)
    # Make sure the input makes sense
    for f in ex.found:
        input_str = f.plugins['posix'].dumps(0)
        unittest.TestCase().assertEqual(input_str.count(b'B'), 10)
...
def run_veritesting_b(arch):
    ...
    for f in ex.found:
        input_str = f.plugins['posix'].dumps(0)
        unittest.TestCase().assertEqual(input_str.count(b'B'), 35)
