...
def run_ddg_global_var_dependencies(arch):
    ...
    for pred in ddg.get_predecessors(tgt_ddg_node):
        ...
        unittest.TestCase().assertFalse(check_dependency(stmt, buf_addr, ord('a')), msg="Target statement has incorrect dependency (DDG is underconstrained)")
    unittest.TestCase().assertTrue(has_correct_dependency, msg='Target statement does not have correct dependency (DDG is overconstrained)')
