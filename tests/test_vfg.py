...
def run_vfg_buffer_overflow(arch):
    ...
    unittest.TestCase().assertGreater(len(vfg.final_states), 0)
    states = vfg.final_states
    unittest.TestCase().assertEqual(len(states), 2)
    stack_check_fail = proj._extern_obj.get_pseudo_addr('symbol hook: __stack_chk_fail')
    unittest.TestCase().assertEqual(set([ s.solver.eval_one(s.ip) for s in states ]),
                            {
                                stack_check_fail,
                                0x4005b4
                            })
    ...
    unittest.TestCase().assertTrue(claripy.backends.vsa.is_true(state.stack_read(12, 4) >= 0x28))
...
def run_vfg_1(arch):
    ...
    unittest.TestCase().assertTrue(vfg_1_addresses[arch].issubset(all_block_addresses))
    ...
    unittest.TestCase().assertTrue(authenticate.addr in vfg.function_final_states)
    authenticate_final_states = vfg.function_final_states[authenticate.addr]
    unittest.TestCase().assertEqual(len(authenticate_final_states), 1)
    authenticate_final_state = next(iter(authenticate_final_states.values()))
    unittest.TestCase().assertIsNotNone(authenticate_final_state)
    unittest.TestCase().assertEqual(authenticate_final_state.solver.eval_upto(authenticate_final_state.regs.rax, 3), [0, 1])

    # optimal execution tests
    # - the basic block after returning from `authenticate` should only be executed once
    unittest.TestCase().assertEqual(vfg._execution_counter[0x4007b3], 1)
    # - the last basic block in `authenticate` should only be executed once (on a non-normalized CFG)
    unittest.TestCase().assertEqual(vfg._execution_counter[0x4006eb], 1)
