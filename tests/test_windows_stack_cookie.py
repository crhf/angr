...
def compare_none(state, test_value):
    ...
    unittest.TestCase().assertEqual(test_value, correct_value)


def compare_random(state, test_value):
    test_value = test_value.concrete
    incorrect_value = angr.simos.windows.VS_SECURITY_COOKIES[state.arch.name].default
    unittest.TestCase().assertNotEqual(test_value, incorrect_value)
...
def compare_symbolic(state, test_value):
    unittest.TestCase().assertTrue(test_value.resolved.symbolic)
...
def test_security_cookie_init():
    ...
    unittest.TestCase().assertRaises(TypeError, project.factory.blank_state, security_cookie_init=1)
